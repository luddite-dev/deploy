...
async fn desired_state_and_observed_status_cross_the_iroh_transport() {
    ...
    let master = Network::bind(master_state.clone())
        .await
        .expect("master network bind");
    let agent = Network::bind(agent_state.clone())
        .await
        .expect("agent network bind");

    timeout(Duration::from_secs(30), master.refresh_identity())
        .await
        .expect("master refresh identity should not hang")
        .expect("master refresh identity");
    timeout(Duration::from_secs(30), agent.refresh_identity())
        .await
        .expect("agent refresh identity should not hang")
        .expect("agent refresh identity");

    master_state
        .push_desired_outbound(DesiredDispatch {
    ...
    agent_state
        .push_observed_outbound(ObservedDispatch {
